...
void GUI::PrintMessage(string msg) const	//Prints a message on status bar
{
	...
	//Added to print multi line
	string temp;
	double H = StatusBarHeight / 1.15;
	for (int i = 0; i < msg.size(); i++)
	{
		if (msg[i] == '\n')
		{
			pWind->DrawString(10, WindHeight - (int)(H), temp);
			H -= 20;
			temp = "";
		}
		else
...
//////////////////////////////////////////////////////////////////////////////////////////
void GUI::PrintWaitingAndAvailble(int WaitingVIP, int WaitingNormal, int WaitingVegan, int AvailableVIP, int AvailableNormal, int AvailableVegan) const
{
	//Ripped from Manual.txt for accessibility 
	...
	//Converting the input integers to char* to concatinate them with the static strings
	string WVIP = to_string(WaitingVIP);
	string WNRM = to_string(WaitingNormal);
	string WVGN = to_string(WaitingVegan);

	string AVIP = to_string(AvailableVIP);
	string ANRM = to_string(AvailableNormal);
	string AVGN = to_string(AvailableVegan);

	//If we want to be fancy, we can update this later so that each order and cook of a certian
	//type are on the same line but start on different x cooridinates (ie, side by side)

	//Printing each line below the other by keeping the x coordinates the same but making the
	//second argument "WindHeight - (int)(StatusBarHeight / Some Value ) Increase by s 
	pWind->DrawString(100, WindHeight - (int)(StatusBarHeight / 1.2), "Waiting VIP orders : " + WVIP);				  	    //Y = 525
	pWind->DrawString(100, WindHeight - (int)(StatusBarHeight / 1.363636364), "Waiting Normal orders : " + WNRM);        //Y = 540
	pWind->DrawString(100, WindHeight - (int)(StatusBarHeight / 1.578947368), "Waiting Vegan orders : " + WVGN);					//Y = 555 
	pWind->DrawString(100, WindHeight - (int)(StatusBarHeight / 1.875), "Available VIP cooks : " + AVIP);				//Y = 570
	pWind->DrawString(100, WindHeight - (int)(StatusBarHeight / 2.307692308), "Available Normal cooks : " + ANRM);					//Y = 585
	pWind->DrawString(100, WindHeight - (int)(StatusBarHeight / 3), "Available Vegan cooks : " + AVGN);					    //Y = 600
...
void GUI::AddToDrawingList(Order* pOrd)
{
	...
	case WAIT:
		...
		break;
	case SRV:
		reg = SRV_REG;	//region of in-service orders
		break;
	case DONE:
		reg = DONE_REG;	//region of serviced orders
		break;
	}
